...
class GhostConv(nn.Module):
    ...
    def __init__(self, c1, c2, k=1, s=1, g=1, act=True):  # ch_in, ch_out, kernel, stride, groups
        ...
        self.cv2 = Conv(c_, c_, 5, s, None, c_, act)
...
class Stem(nn.Module): #32->64
    def __init__(self, c1, e=0.5, f=2):  # ch_in, ch_out, shortcut, groups, expansion
        super(Stem, self).__init__()
        c_ = int(c1 * e)  # hidden channels
        c2 = int(c1 * f)
        self.cv1 = Ghost(c1, c_, 1, 1)  # 左侧1x1降维
        self.cv2 = Ghost(c1, c_, 1, 1)  # 右侧1x1降维
        self.cv3 = Ghost(c_, c_, 3, 1) # 右侧3x3不变
        self.cv4 = Ghost(c_, c_, 3, 1) # 右侧3x3不变
        self.cv5 = Conv(c2, c1, 1, 1)
        #self.att = CA_Block(c1)
...
class C2(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        ...
        #self.attention = CBAMBlock(2 * self.c)  # or SpatialAttention()
...
# CSP Bottleneck with 2 convolutions
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        super().__init__(c1, c2, n, shortcut, g, e)
        self.c = int(c2 * e)  # hidden channels
        self.m = nn.Sequential(*[Ghost(self.c, self.c) for _ in range(n)])
